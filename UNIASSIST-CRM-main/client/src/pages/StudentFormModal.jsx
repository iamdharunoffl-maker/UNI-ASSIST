...
export const StudentFormModal = ({ isOpen, onClose, student, onSave }) => {
  ...
  useEffect(() => {
    const fetchMasters = async () => {
      try {
        ...
        setAllUniversities(unisData.filter(u => u.status === 'Active'));
        setAllCourses(coursesData.filter(c => c.status === 'Active'));
      } catch (err) {
  ...
  useEffect(() => {
    if (selectedUniversity) {
      const filtered = allCourses.filter(c => c.university === selectedUniversity);
