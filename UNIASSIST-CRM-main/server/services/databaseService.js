...
const migrateFromExcel = async (excelPath) => {
  ...
  try {
    ...
    if (coursesSheet) {
      ...
      const stmt = await conn.prepare('INSERT INTO courses (name) VALUES (?)');
      const coursePromises = [];
      coursesSheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
        if (rowNumber === 1) return;
        const name = (row.values[2] || row.values[1] || '').toString().trim();
        if (name && name !== 'ID' && name !== 'Name') {
          coursePromises.push(stmt.run(name));
        }
      });
...
const readSheet = async (sheetName) => {
  ...
  if (name === 'courses') {
    return await conn.all('SELECT id, name, status FROM courses ORDER BY name');
  }
...
// Courses
const insertCourse = async (name, status = 'Active') => {
  const conn = await openDb();
  const result = await conn.run('INSERT INTO courses (name, status) VALUES (?, ?)', [name, status]);
  return { id: result.lastID, name, status };
};
...
const updateCourseById = async (id, fields) => {
  const conn = await openDb();
  const status = (fields.status || 'active').toString();
  const result = await conn.run('UPDATE courses SET name=?, status=? WHERE id=?', [fields.name, status, id]);
  return result.changes > 0;
};
