...
import { HiPlus, HiTrash, HiCheck, HiXMark, HiPencilSquare } from 'react-icons/hi2';
...
export const MastersPage = () => {
  ...
  const onToggleCountryStatus = async (item) => {
    try {
      const newStatus = isActive(item) ? 'Inactive' : 'Active';
  ...
  const onAddCourse = async (data) => {
    try {
      const payload = { name: data.name, university: data.university, status: data.status || 'Active' };
      if (editingCourse) {
        await masterService.updateCourse(editingCourse.id, payload);
        toast.success('Course updated.');
        setEditingCourse(null);
      } else {
        await masterService.createCourse(payload);
  ...
  const onEditCourse = (item) => {
    ...
    resCourse({ name: item.name || '', university: item.university || '', status: item.status || 'Active' });
  };
                      ...
                      countries.map(item => (
                        <tr key={item.id} className="hover:bg-slate-50/50">
                          ...
                          <td className="px-6 py-3.5">
                            ...
                            >
                              {isActive(item) ? <HiCheck className="w-3 h-3" /> : <HiXMark className="w-3 h-3" />}
                ...
                <form onSubmit={subUni(onAddUni)} className="flex flex-col gap-4">
                  <div className="flex flex-col gap-1.5">
                    ...
                    >
                      ...
                      {countries.filter(isActive).map(c => (
                ...
                <form onSubmit={subCourse(onAddCourse)} className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    >
                      ...
                      {universities.filter(isActive).map(u => (
                  ...
                  <div className="flex gap-2">
                    <button
                      type="submit"
                      className="flex-1 inline-flex items-center justify-center gap-1.5 px-4 py-2.5 bg-brand-600 hover:bg-brand-700 text-sm font-semibold text-white rounded-xl shadow-md transition-all focus:outline-none"
                    >
                      <HiPlus className="w-4 h-4" />
                    ...
                    {editingCourse && (
                      <button
                        ...
                        className="inline-flex items-center justify-center gap-1.5 px-4 py-2.5 bg-white border border-slate-200 text-sm font-semibold text-slate-700 rounded-xl shadow-sm hover:bg-slate-50 transition-all"
                      >
                        Cancel
                      </button>
                    )}
